...
public class AllocationApiFilterIntegrationTest extends BaseIntegrationTest {
    ...
    void testFilterAllAllocations() {
        ...
        assertEquals(allocation1.getSubject(), allocationDTOList.getFirst().getSubject());
    ...
    void testFilterAllocationsByEmployeeEmail() {
        var room = roomRepository.saveAndFlush(newRoomBuilder().build());
        var employee1 = newEmployeeBuilder().email(DEFAULT_ALLOCATION_EMPLOYEE_EMAIL + 1).build();
        var employee2 = newEmployeeBuilder().email(DEFAULT_ALLOCATION_EMPLOYEE_EMAIL + 2).build();
        ...
        assertEquals(allocation1.getId(), allocationDTOList.getFirst().getId());
    ...
    void testFilterAllocationsByRoomId() {
        ...
        var allocation1 = allocationRepository.saveAndFlush(newAllocationBuilder(roomA).subject(DEFAULT_ALLOCATION_SUBJECT + 1).build());
        var allocation2 = allocationRepository.saveAndFlush(newAllocationBuilder(roomA).subject(DEFAULT_ALLOCATION_SUBJECT + 2).build());
        allocationRepository.saveAndFlush(newAllocationBuilder(roomB).subject(DEFAULT_ALLOCATION_SUBJECT + 3).build());
    ...
    void testFilterAllocationsByDateRange() {
        ...
        allocationRepository.saveAndFlush(newAllocationBuilder(room).startAt(baseEndAt.plusDays(1)).endAt(baseEndAt.plusDays(3).plusHours(1)).build());
    ...
    @Test
    void testFilterAllocationPagination() {
    ...
    @Test
    void testFilterAllocationPaginationAndLimit() {
    ...
    void testFilterAllocationOrderByStartAtDesc() {
        ...
        var allocationsListDTO = api.listAllocations(TEST_CLIENT_API_KEY, null, null, null, null, "-startAt", null, null);

        assertEquals(3, allocationsListDTO.size());
        assertEquals(allocationsList.getFirst().getId(), allocationsListDTO.get(2).getId());
        assertEquals(allocationsList.get(1).getId(), allocationsListDTO.get(1).getId());
        assertEquals(allocationsList.get(2).getId(), allocationsListDTO.get(0).getId());
