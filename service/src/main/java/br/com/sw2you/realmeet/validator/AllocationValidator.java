...
public class AllocationValidator {
    ...
    private static void validateSubject(String subject, ValidationErrors validationErrors) {
        if (validateRequired(subject, ALLOCATION_SUBJECT, validationErrors)) {
    ...
    private static void validateEmployeeName(String employeeName, ValidationErrors validationErrors) {
        if (validateRequired(employeeName, ALLOCATION_EMPLOYEE_NAME, validationErrors)) {
    ...
    private static void validateEmployeeEmail(String employeeEmail, ValidationErrors validationErrors) {
        if (validateRequired(employeeEmail, ALLOCATION_EMPLOYEE_EMAIL, validationErrors)) {
            validateMaxLength(employeeEmail, ALLOCATION_EMPLOYEE_EMAIL, ALLOCATION_EMPLOYEE_EMAIL_MAX_LENGTH, validationErrors);
        }
    }

    private static void validateDates(OffsetDateTime startAt, OffsetDateTime endAt, ValidationErrors validationErrors) {
    ...
    private static boolean validateDatesPresent(OffsetDateTime startAt, OffsetDateTime endAt, ValidationErrors validationErrors) {
        return (
    ...
    private static void validateDateOrdering(OffsetDateTime startAt, OffsetDateTime endAt, ValidationErrors validationErrors) {
    ...
    private static void validateDateInTheFuture(OffsetDateTime date, ValidationErrors validationErrors) {
        if (date.isBefore(now())) {
            validationErrors.add(ALLOCATION_START_AT, ALLOCATION_START_AT + IN_THE_PAST);
        }
    }

    private static void validateDuration(OffsetDateTime startAt, OffsetDateTime endAt, ValidationErrors validationErrors) {
    ...
    private static void validateIfTimeAvailable(OffsetDateTime startAt, OffsetDateTime endAt, ValidationErrors validationErrors) {
        // TODO
    }
}
