...
public class AllocationReportHandler extends AbstractReportHandler<Allocation, AllocationReportData> {
    ...
    protected List<Allocation> fetchReportData(AllocationReportData reportData) {
        return allocationRepository.findWithFilters(null, null,
                reportData.getDateFrom()
                        .atTime(OffsetTime.MIN), reportData.getDateTo().atTime(OffsetTime.MAX));
    }
        ...
        return ((jrField, allocation) -> {
            switch (jrField.getName()) {
                case FIELD_DATE_FROM:
                    return DateUtils.formatUsingDateTimePattern(allocation.getStartAt());
                case FIELD_DATE_TO:
                    return DateUtils.formatUsingDateTimePattern(allocation.getEndAt());
